...
export default function Card({ content }: { content: BirthdayContent }) {
  ...
  return (
    <div className="relative bg-white/80 backdrop-blur-lg rounded-3xl shadow-[0_10px_40px_rgba(0,0,0,0.2)] p-8 text-center max-w-md w-full border border-white/30">
      <div className="absolute inset-0 pointer-events-none rounded-3xl bg-gradient-to-r from-pink-400/20 via-purple-400/20 to-yellow-400/20 blur-xl opacity-50"></div>
        ...
        {step === 0 && <p className="text-lg md:text-xl text-gray-700 mb-6 leading-relaxed">{content.intro}</p>}
        ...
        {current.type === "tease" && (
          <>
            <p className="text-lg md:text-xl text-gray-700 mb-6 leading-relaxed">{current.text}</p>
            <button
              onClick={nextStep}
